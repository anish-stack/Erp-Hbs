...
async function bootstrap() {
  for (const dir of [config.bulk.exportDir, config.bulk.importDir]) {
...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down user service', signal);
...
bootstrap().catch((err) => {
  logger.error('User service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
