...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down finance service', signal);
  ...
  try {
    ...
    logger.info('Finance service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Finance service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
