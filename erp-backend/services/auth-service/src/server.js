...
env.requireAll(['DATABASE_URL', 'JWT_ACCESS_SECRET', 'JWT_REFRESH_SECRET', 'RABBITMQ_URL']);
...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down auth service', signal);
...
bootstrap().catch((err) => {
  logger.error('Auth service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
