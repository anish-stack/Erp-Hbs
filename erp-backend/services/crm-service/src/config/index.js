...
module.exports = {
  ...
  lead: {
    staleDays: env.int("LEAD_STALE_DAYS", 14),
    followUpCron: env.str("FOLLOWUP_SCAN_CRON", "0 8 * * *"),
  },
  internal: {
    masterServiceUrl: env.str(
      "MASTER_SERVICE_URL",
      "http://master-service:4004",
    ),
  ...
  local: {
    root: path.resolve(
      serviceRoot,
      env.str("LOCAL_STORAGE_PATH", "./storage/files"),
    ),
    tmp: path.resolve(serviceRoot, env.str("LOCAL_TMP_PATH", "./storage/tmp")),
    signingSecret: env.str(
      "LOCAL_SIGNING_SECRET",
      "change_me_local_signing_secret",
    ),
  },
