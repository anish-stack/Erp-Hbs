...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down Purchase service', signal);
  ...
  try {
    ...
    logger.info('Purchase service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Purchase service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
