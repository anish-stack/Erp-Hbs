...
function createApp() {
  ...
  app.use(healthRouter({
    ...
    checks: { database: isHealthy, masterService: async () => MasterClient.healthy(), inventoryService: async () => InventoryClient.healthy() }
  }));
  app.get('/', (req, res) => ApiResponse.ok(res, { service: config.serviceName, version: config.version, docs: '/docs' }, 'ERP Quality Inspection Service'));
