...
class MasterClient {
  static async nextPoCode(user) {
    const result = await call(config.internal.masterServiceUrl, `${config.basePath}/master/sequences/PURCHASE_ORDER/next`, { method: 'POST', body: { count: 1 }, user });
    return result.first;
  }

  static async nextGrnCode(user) {
    const result = await call(config.internal.masterServiceUrl, `${config.basePath}/master/sequences/GRN/next`, { method: 'POST', body: { count: 1 }, user });
    return result.first;
  }
