...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down supplier service', signal);
  ...
  try {
    ...
    logger.info('Supplier service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Supplier service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
