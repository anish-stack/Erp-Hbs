...
function assertUsable(user) {
  ...
  if (!user.role || !user.role.isActive) {
    throw ApiError.forbidden(
      "Assigned role is inactive. Contact your administrator",
    );
  }
}
...
class AuthService {
  static async register(payload, context = {}, actorId = null) {
    ...
    if (!role)
      throw ApiError.badRequest("Role not found or inactive", {
        field: "roleId",
      });
  ...
  static async login({ email, password }, context = {}) {
    ...
    if (user.lockedUntil && user.lockedUntil > new Date()) {
      ...
      throw ApiError.forbidden(
        `${MESSAGES.AUTH.ACCOUNT_LOCKED}. Try again in ${minutes} minute(s)`,
      );
    }
    ...
    if (!passwordValid) {
      ...
      await AuthLogRepository.record({
        ...
        email: user.email,
        event: shouldLock
          ? AUTH_LOG_EVENT.ACCOUNT_LOCKED
          : AUTH_LOG_EVENT.LOGIN,
        success: false,
        reason: shouldLock
          ? "ACCOUNT_LOCKED"
          : `INVALID_PASSWORD (${attempts})`,
        ipAddress: context.ipAddress,
        userAgent: context.userAgent,
      });
  ...
  static async refresh(refreshToken, context = {}) {
    ...
    if (session.revokedAt) {
      await TokenService.revokeFamily(
        session.familyId,
        REVOKE_REASON.REUSE_DETECTED,
      );
      await TokenService.revokeAllSessions(
        session.userId,
        REVOKE_REASON.REUSE_DETECTED,
      );
      ...
      await publisher.tokenRevoked(session.userId, {
        reason: REVOKE_REASON.REUSE_DETECTED,
      });
    ...
    if (session.accessJti)
      await TokenService.blacklistAccessJti(session.accessJti);
  ...
  static async logoutAll(userId, context = {}) {
    const revoked = await TokenService.revokeAllSessions(
      userId,
    ...
    await publisher.tokenRevoked(userId, {
      reason: REVOKE_REASON.LOGOUT_ALL,
      revoked,
    });
  ...
  static async changePassword(
    ...
    await UserRepository.setPassword(
      userId,
      await utils.password.hash(newPassword),
      userId,
    );
    await TokenService.revokeAllSessions(
      userId,
  ...
  static async verifyOtp(
    { email, code, purpose = OTP_PURPOSE.LOGIN },
    context = {},
  ) {
