...
function createApp() {
  ...
  app.use(healthRouter({
    ...
    checks: { database: isHealthy, salesService: async () => SalesClient.healthy(), purchaseService: async () => PurchaseClient.healthy() }
  }));
  app.get('/', (req, res) => ApiResponse.ok(res, { service: config.serviceName, version: config.version, docs: '/docs' }, 'ERP Finance Service'));
