...
const skipBodyParse = (req) =>
  ...
  req.originalUrl.startsWith(`${config.apiPrefix}/files`);
...
app.use((req, res, next) => {
  ...
  return middlewares.xssSanitizer(req, res, next);
});
...
app.use((req, res, next) => {
  ...
  return middlewares.requestLogger(req, res, next);
});
  app.use(
    healthRouter({
  ...
  app.use(
    "/docs",
    swaggerUi.serve,
    swaggerUi.setup(buildGatewayDocument(), swaggerUiOptions),
  );
