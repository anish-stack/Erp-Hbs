...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down RFQ service', signal);
  ...
  try {
    ...
    logger.info('RFQ service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('RFQ service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
