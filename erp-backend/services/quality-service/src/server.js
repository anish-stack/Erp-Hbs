...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down quality service', signal);
  ...
  try {
    ...
    logger.info('Quality service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Quality service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
