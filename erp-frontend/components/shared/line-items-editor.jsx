...
export function LineItemsEditor({ value = [], onChange }) {
  ...
  const lineTotal = (row) => {
    ...
    return afterDiscount + (afterDiscount * (Number(row.taxRatePct) || 0)) / 100;
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Button type="button" size="sm" variant="outline" onClick={addRow}><Plus className="h-3.5 w-3.5" /> Add line</Button>
      </div>

      {value.length === 0 && (
        <p className="rounded-md border border-dashed border-border p-4 text-center text-sm text-muted-foreground">No lines yet — add a part to begin.</p>
      )}

      <div className="space-y-3">
        {value.map((row, idx) => (
          <div key={idx} className="grid grid-cols-12 items-end gap-2 rounded-md border border-border p-3">
            ...
            <div className="col-span-6 sm:col-span-1 space-y-1">
              ...
              <Input type="number" min="0" max="100" value={row.discountPct} onChange={(e) => updateRow(idx, { discountPct: e.target.value })} />
            </div>
            <div className="col-span-6 sm:col-span-1 space-y-1">
              <Label className="text-xs">Tax %</Label>
              <Input type="number" min="0" max="100" value={row.taxRatePct} onChange={(e) => updateRow(idx, { taxRatePct: e.target.value })} />
            </div>
            <div className="col-span-10 sm:col-span-1 space-y-1">
              <Label className="text-xs">Total</Label>
              <p className="pt-1.5 text-sm font-medium tabular-nums">{formatMoney(lineTotal(row))}</p>
            </div>
