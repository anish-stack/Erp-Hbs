...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down warehouse service', signal);
  ...
  try {
    ...
    logger.info('Warehouse service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Warehouse service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
