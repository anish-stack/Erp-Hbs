...
function createApp() {
  ...
  app.use(healthRouter({ serviceName: config.serviceName, version: config.version, checks: { database: isHealthy } }));
  app.get('/', (req, res) => ApiResponse.ok(res, { service: config.serviceName, version: config.version, docs: '/docs', socket: '/socket.io' }, 'ERP Notification Service'));
