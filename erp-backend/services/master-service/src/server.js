...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down master service', signal);
  ...
  try {
    ...
    logger.info('Master service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Master service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
