...
function createApp() {
  ...
  app.use(healthRouter({
    ...
    checks: {
      database: isHealthy,
      salesService: async () => SalesClient.healthy(),
      warehouseService: async () => WarehouseClient.healthy(),
      inventoryService: async () => InventoryClient.healthy()
    }
  }));
  app.get('/', (req, res) => ApiResponse.ok(res, { service: config.serviceName, version: config.version, docs: '/docs' }, 'ERP Shipment Service'));
