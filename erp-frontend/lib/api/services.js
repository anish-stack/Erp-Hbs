...
const update = (path) => (id, body) => api.put(`${path}/${id}`, body).then(unwrap);
const remove = (path) => (id) => api.delete(`${path}/${id}`).then(unwrap);
const action = (path) => (id, verb, body) => api.post(`${path}/${id}/${verb}`, body || {}).then(unwrap);

/* ------------------------------- Auth ------------------------------- */
export const authApi = {
  login: (body) => api.post('/auth/login', body).then((r) => r.data?.data || r.data),
  sendOtp: (body) => api.post('/auth/send-otp', body).then(unwrap),
  verifyOtp: (body) => api.post('/auth/verify-otp', body).then((r) => r.data?.data || r.data),
...
export const dashboardApi = {
  ...
  saveLayout: (widgetKeys) => api.put('/dashboard/layout', { widgetKeys }).then(unwrap)
};
...
export const rolesApi = {
  ...
  setPermissions: (id, permissionCodes) => api.put(`/roles/${id}/permissions`, { permissionCodes }).then(unwrap),
  addPermissions: (id, permissions) => api.post(`/roles/${id}/permissions`, { permissions }).then(unwrap),
  removePermissions: (id, permissions) => api.delete(`/roles/${id}/permissions`, { data: { permissions } }).then(unwrap),
...
export const leadsApi = {
  ...
  setStage: (id, stage) => api.patch(`/leads/${id}/stage`, { stage }).then(unwrap),
  followUp: (id, body) => api.post(`/leads/${id}/followup`, body).then(unwrap),
  convert: (id, body) => api.post(`/leads/${id}/convert`, body || {}).then(unwrap),
  activities: (id, params) => api.get(`/leads/${id}/activities`, { params }).then(unwrapList)
};
export const activitiesApi = {
  create: create('/activities'),
  complete: (id, body) => api.post(`/activities/${id}/complete`, body || {}).then(unwrap)
};
...
export const purchaseApi = {
  ...
  reject: (id, reason) => api.post(`/purchase/${id}/reject`, { reason }).then(unwrap),
  issue: (id) => api.post(`/purchase/${id}/issue`).then(unwrap),
  cancel: (id, reason) => api.post(`/purchase/${id}/cancel`, { reason }).then(unwrap),
  ...
  createGrn: (poId, body) => api.post(`/purchase/${poId}/grns`, body).then(unwrap)
};
...
export const inventoryApi = {
  ...
  lowStock: (params) => api.get('/inventory/low-stock', { params }).then(unwrapList),
  movements: (params) => api.get('/inventory/movements', { params }).then(unwrapList),
  reservations: (params) => api.get('/inventory/reservations', { params }).then(unwrapList)
};
...
export const warehouseApi = {
  ...
  createZone: (id, body) => api.post(`/warehouse/${id}/zones`, body).then(unwrap),

  bins: (id, params) => api.get(`/warehouse/${id}/bins`, { params }).then(unwrapList),
  createBin: (id, body) => api.post(`/warehouse/${id}/bins`, body).then(unwrap),
  blockBin: (binId, reason) => api.post(`/warehouse/bins/${binId}/block`, { reason }).then(unwrap),
  unblockBin: (binId) => api.post(`/warehouse/bins/${binId}/unblock`).then(unwrap),

  tasks: (params) => api.get('/warehouse/tasks', { params }).then(unwrapList),
  createTask: create('/warehouse/tasks'),
  assignTask: (taskId, userId) => api.post(`/warehouse/tasks/${taskId}/assign`, { userId }).then(unwrap),
  startTask: (taskId) => api.post(`/warehouse/tasks/${taskId}/start`).then(unwrap),
  completeTask: (taskId, body) => api.post(`/warehouse/tasks/${taskId}/complete`, body || {}).then(unwrap),
  cancelTask: (taskId, reason) => api.post(`/warehouse/tasks/${taskId}/cancel`, { reason }).then(unwrap)
};

/* ------------------------------ Quality ----------------------------- */
export const qualityApi = {
  inspections: (params) => api.get('/quality/inspections', { params }).then(unwrapList),
  inspection: getOne('/quality/inspections'),
  createInspection: create('/quality/inspections'),
  startInspection: (id) => api.post(`/quality/inspections/${id}/start`).then(unwrap),
  submitResults: (id, results) => api.post(`/quality/inspections/${id}/results`, { results }).then(unwrap),
  completeInspection: (id, body) => api.post(`/quality/inspections/${id}/complete`, body || {}).then(unwrap),
  holdInspection: (id, reason) => api.post(`/quality/inspections/${id}/hold`, { reason }).then(unwrap),
  cancelInspection: (id, reason) => api.post(`/quality/inspections/${id}/cancel`, { reason }).then(unwrap),
...
export const salesApi = {
  ...
  quotations: (params) => api.get('/sales/quotations', { params }).then(unwrapList),
...
export const financeApi = {
  ...
  invoiceFromSalesOrder: (orderId) => api.post('/finance/invoices/from-sales-order', { orderId }).then(unwrap),
  payments: (params) => api.get('/finance/payments', { params }).then(unwrapList),
...
export const shipmentApi = {
  ...
  fromOrder: (orderId) => api.post('/shipment/from-order', { orderId }).then(unwrap),
  ...
  dispatch: (id, body) => api.post(`/shipment/${id}/dispatch`, body).then(unwrap),
  deliver: (id) => api.post(`/shipment/${id}/deliver`).then(unwrap),
  cancel: (id, reason) => api.post(`/shipment/${id}/cancel`, { reason }).then(unwrap),
...
export const filesApi = {
  upload: (file, extra = {}) => {
    ...
    return api.post('/files/upload', form, { headers: { 'Content-Type': 'multipart/form-data' } }).then(unwrap);
  },
  downloadUrl: (id) => `${process.env.NEXT_PUBLIC_API_BASE || ''}/api/v1/files/${id}/download`
};
