...
const CustomerImportExportService = require("../services/customer-import-export.service");
class CustomerController {
  ...
  static export = asyncHandler(async (req, res) => {
    const buffer = await CustomerImportExportService.exportCustomers(req.query);
  ...
  static exportTemplate = asyncHandler(async (req, res) => {
    const buffer = await CustomerImportExportService.exportTemplate();
  ...
  static import = asyncHandler(async (req, res) => {
    ...
    const result = await CustomerImportExportService.importCustomers(
      req.file.buffer,
      req.user,
      {
    ...
    return ApiResponse.ok(res, result, "Customer import completed");
  });
  ...
  static adjustCredit = asyncHandler(async (req, res) => {
    const result = await CustomerService.adjustCredit(
      req.params.id,
      req.body,
      req.user,
    );
  ...
  static checkCredit = asyncHandler(async (req, res) => {
    const result = await CustomerService.checkCreditAvailability(
      req.params.id,
      req.query.amount ? Number(req.query.amount) : Number(req.body.amount),
    );
    return ApiResponse.ok(
      res,
      result,
      result.allowed ? "Credit available" : "Credit limit would be exceeded",
    );
  });
  ...
  static removeAddress = asyncHandler(async (req, res) => {
    const result = await CustomerService.removeAddress(
      req.params.id,
      req.params.childId,
    );
  ...
  static removeContact = asyncHandler(async (req, res) => {
    const result = await CustomerService.removeContact(
      req.params.id,
      req.params.childId,
    );
  ...
  static activities = asyncHandler(async (req, res) => {
    const result = await ActivityService.listForCustomer(
      req.params.id,
      req.query,
    );
