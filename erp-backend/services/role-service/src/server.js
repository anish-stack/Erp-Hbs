...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down role service', signal);
  ...
  try {
    ...
    logger.info('Role service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Role service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
