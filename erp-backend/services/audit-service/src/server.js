...
async function bootstrap() {
  if (!fs.existsSync(config.export.dir)) fs.mkdirSync(config.export.dir, { recursive: true });
  ...
  server = app.listen(config.port, () => {
    logger.info(
      '%s listening on port %d [%s] capturing all events',
      config.serviceName,
      config.port,
      config.nodeEnv
    );
...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down audit service', signal);
...
bootstrap().catch((err) => {
  logger.error('Audit service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
