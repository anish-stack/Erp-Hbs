...
module.exports = {
  ...
  list: Joi.object({
    ...
    sortBy: Joi.string().valid('createdAt', 'companyName', 'estimatedValue', 'nextFollowUpAt'),
  ...
  create: Joi.object({
    ...
    source: Joi.string().valid(...Object.values(LEAD_SOURCE)).default('OTHER'),
  ...
  changeStage: Joi.object({
    stage: Joi.string().valid(...Object.values(LEAD_STAGE)).required(),
    reason: Joi.string().max(500).allow('', null),
    lostReason: Joi.string().max(500).when('stage', { is: 'LOST', then: Joi.required() })
  }),
  ...
  convert: Joi.object({
    ...
    type: Joi.string().valid('BUSINESS', 'INDIVIDUAL', 'GOVERNMENT').default('BUSINESS'),
    gstin: Joi.string().length(15).uppercase().allow('', null),
    pan: Joi.string().length(10).uppercase().allow('', null),
    taxTreatment: Joi.string().valid('REGISTERED', 'COMPOSITION', 'UNREGISTERED', 'OVERSEAS', 'SEZ').default('REGISTERED'),
    email: Joi.string().email().allow('', null),
    phone: Joi.string().max(30).allow('', null),
    currencyCode: Joi.string().length(3).uppercase(),
    paymentTermDays: Joi.number().integer().min(0).max(365).default(30),
    creditLimit: Joi.number().min(0).default(0),
    segment: Joi.string().valid('ENTERPRISE', 'SMB', 'STARTUP', 'GOVERNMENT', 'RETAIL').default('SMB')
  })
};
