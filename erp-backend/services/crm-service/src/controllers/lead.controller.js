...
class LeadController {
  ...
  static importExcel = asyncHandler(async (req, res) => {
    console.log(req.file)
    const result = await LeadService.importExcel(req.file, req.user);

    return ApiResponse.ok(res, result, "Leads imported successfully");
  });

  static exportExcel = asyncHandler(async (req, res) => {
    console.log(req)
    const buffer = await LeadService.exportExcel(req.query);
  ...
  static convert = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.created(
      res,
      result,
      `Lead converted to customer ${result.customer.code}`,
    );
  });
