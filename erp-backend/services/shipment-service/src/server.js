...
async function shutdown(signal) {
  logger.warn('Received %s, shutting down shipment service', signal);
  ...
  try {
    ...
    logger.info('Shipment service shutdown complete');
    process.exit(0);
  } catch (err) {
...
bootstrap().catch((err) => {
  logger.error('Shipment service failed to start: %s', err.stack || err.message);
  process.exit(1);
});
