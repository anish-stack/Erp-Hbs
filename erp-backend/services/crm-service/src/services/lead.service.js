...
const { LEAD_STAGE, STAGE_TRANSITIONS, STAGE_PROBABILITY, CACHE } = require('../constants');
...
function shape(lead) {
  return {
    ...
    weightedValue: lead.estimatedValue
      ? Number((Number(lead.estimatedValue) * (lead.probability / 100)).toFixed(2))
      : null,
    city: lead.city,
    state: lead.state,
...
function assertTransition(from, to) {
  ...
  if (!allowed.includes(to)) {
    throw ApiError.badRequest(`A lead cannot move from ${from} to ${to}`, { currentStage: from, allowedNext: allowed });
  }
}
...
class LeadService {
  static async list(query) {
    const pagination = utils.pagination.buildPagination(query, {
      allowedSortFields: ['createdAt', 'companyName', 'estimatedValue', 'nextFollowUpAt'],
    ...
    const where = {
      ...
      ...(query.search
        ? { OR: [{ companyName: { contains: query.search } }, { contactName: { contains: query.search } }, { email: { contains: query.search } }] }
        : {})
    };

    const { items, total } = await LeadRepository.paginate({ where, skip: pagination.skip, take: pagination.take, orderBy: pagination.orderBy });
  ...
  static async create(payload, user) {
    const code = `LEAD-${Date.now().toString(36).toUpperCase()}-${randomUUID().slice(0, 4).toUpperCase()}`;
    ...
    await LeadRepository.logStage({ leadId: lead.id, fromStage: null, toStage: LEAD_STAGE.NEW, reason: 'Lead created', actorId: user.id });
  ...
  static async update(id, payload, user) {
    ...
    if (['WON', 'LOST'].includes(existing.stage) && payload.stage === undefined) {
  ...
  static async changeStage(id, toStage, { reason, lostReason } = {}, user) {
    ...
    await LeadRepository.logStage({ leadId: id, fromStage: existing.stage, toStage, reason: reason || lostReason || null, actorId: user.id });
    await cache.del(CACHE.pipeline());
  ...
  static async logFollowUp(id, notes, nextFollowUpAt, user) {
    const existing = await LeadRepository.findById(id);
    if (!existing) throw ApiError.notFound('Lead not found');

    await RelationRepository.createActivity({
      leadId: id, type: 'CALL', subject: 'Follow-up logged', notes: notes || null,
      completedAt: new Date(), createdBy: user.id
    });
  ...
  static async pipeline() {
    return cache.remember(CACHE.pipeline(), 300, async () => {
      ...
      for (const row of rows) {
        byStage[row.stage] = { count: row._count._all, value: Number(row._sum.estimatedValue || 0) };
      }

      const openStages = ['NEW', 'CONTACTED', 'QUALIFIED', 'PROPOSAL', 'NEGOTIATION'];
      const openValue = openStages.reduce((sum, stage) => sum + byStage[stage].value, 0);
      const openCount = openStages.reduce((sum, stage) => sum + byStage[stage].count, 0);
      const winRate = (byStage.WON.count + byStage.LOST.count) > 0
        ? Number(((byStage.WON.count / (byStage.WON.count + byStage.LOST.count)) * 100).toFixed(1))
        : 0;
